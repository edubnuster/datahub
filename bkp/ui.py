...
class ConfigWindow(SimpleDialog):
    def __init__(self, master, config_data: Dict[str, Any], on_save):
        ...
        super().__init__(master, "Configuração de conexão", "520x390")
        self._build()
    def _build(self):
        frm = ttk.Frame(self, padding=14)
        frm.pack(fill="both", expand=True)
        ttk.Label(frm, text="Configuração do banco de dados", font=("Segoe UI", 11, "bold")).pack(anchor="w", pady=(0, 6))
        ttk.Label(frm, text="Informe os dados de conexão com o banco.", wraplength=470, justify="left").pack(anchor="w", pady=(0, 12))
        form = ttk.Frame(frm)
        form.pack(fill="x")
        self.entries = {}
        ...
        for i, (label, key) in enumerate(fields):
            ...
            entry = ttk.Entry(form, width=42, show="*" if key == "password" else None)
        ...
        ttk.Button(btns, text="Testar conexão", command=self._test).pack(side="left")
...
class OpenInvoicesWindow(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Button(actions, text="Configuração", command=self.open_config).pack(side="left", padx=(12, 0))
